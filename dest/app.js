...
var React, Snap, _, _y, active, adjacencyMatrix, bfs, bfsButtonRect, bfsButtonStroke, bfsButtonText, bfs_waitChoosingNode, dark, dfs, dfsButtonRect, dfsButtonStroke, dfsButtonText, dfs_waitChoosingNode, ex, existLink, findComps, findCompsButtonRect, findCompsButtonStroke, findCompsButtonText, gap, humane, inactive, light, messageIsShown, mousedownCoords, nodeStyle, nodes, paper, path, pathArray, pathStyle, prevNode, prevNodeStyle, resetGraphAnimation, side, start, startBfs, startDfs, tableArray, timeDfs, timers, updatePath, updateTable;
...
dfs_waitChoosingNode = false;

bfs_waitChoosingNode = false;
...
path = paper.path("").attr(pathStyle);

mousedownCoords = void 0;
...
existLink = function(node1, node2, callback) {
  var j, len, pair;
  for (j = 0, len = pathArray.length; j < len; j++) {
    pair = pathArray[j];
    if (pair.from === node1 && pair.to === node2 || pair.from === node2 && pair.to === node1) {
...
updateTable = function() {
  var e, exist, i, indicator, j, k, l, len, ref, ref1, results, x, xnumber, y, ynumber;
  for (j = 0, len = tableArray.length; j < len; j++) {
    e = tableArray[j];
    e.remove();
  }
  for (i = k = 0, ref = nodes.length; 0 <= ref ? k <= ref : k >= ref; i = 0 <= ref ? ++k : --k) {
    ynumber = paper.text(10, i * (side + gap) + 8, i);
    xnumber = paper.text(i * (side + gap), start - gap, i);
    tableArray.push(ynumber, xnumber);
  }
  results = [];
  for (x = l = 0, ref1 = nodes.length; 0 <= ref1 ? l < ref1 : l > ref1; x = 0 <= ref1 ? ++l : --l) {
    results.push((function() {
      var m, ref2, results1;
      results1 = [];
      for (y = m = 0, ref2 = nodes.length; 0 <= ref2 ? m < ref2 : m > ref2; y = 0 <= ref2 ? ++m : --m) {
        exist = existLink(nodes[x], nodes[y]);
        adjacencyMatrix[x] || (adjacencyMatrix[x] = []);
        adjacencyMatrix[x][y] = exist;
        indicator = paper.rect(start + x * (side + gap), start + y * (side + gap), side, side, 5).attr(exist ? active : inactive).data("x", x).data("y", y).click(function() {
          var from, to;
          from = nodes[this.data("x")];
          to = nodes[this.data("y")];
          if (existLink(from, to)) {
            pathArray = pathArray.filter(function(e) {
              return (!_.isEqual(e, {
                from: from,
                to: to
              })) && (!_.isEqual(e, {
        ...
        results1.push(tableArray.push(indicator));
      }
      return results1;
    })());
  }
...
updatePath = function() {
  var from, j, len, pathPart, pathString, to;
  pathString = "";
  for (j = 0, len = pathArray.length; j < len; j++) {
...
dfs = function(start, withAnimation) {
  var i, j, ref, timer;
  if (withAnimation === void 0) {
    withAnimation = true;
  }
  ex.push(start);
  for (i = j = 0, ref = adjacencyMatrix.length; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
    if (ex.indexOf(i) === -1 && adjacencyMatrix[start][i]) {
      if (withAnimation) {
...
resetGraphAnimation = function() {
  var j, k, len, len1, node, results, timer;
  ...
  ex = [];
  results = [];
  for (k = 0, len1 = timers.length; k < len1; k++) {
    timer = timers[k];
...
dfsButtonStroke = paper.rect(0, 0, 190, 40, 5).attr({
  fill: "none",
  stroke: light,
  strokeWidth: 2
});

dfsButtonRect = paper.rect(0, 0, 190, 40, 5).attr({
  fill: light,
  opacity: 0,
  strokeWidth: 2
});

dfsButtonText = paper.text(24, 25, "Depth First Search").attr({
  fill: light,
  fontSize: 18,
  fontWeight: 200
});

paper.group(dfsButtonRect, dfsButtonStroke, dfsButtonText).attr({
  transform: "t" + (screen.width - 200) + ",10"
}).hover(function() {
  dfsButtonRect.animate({
    opacity: 1
  }, 200, mina.linear);
  return dfsButtonText.animate({
    fill: dark
  }, 200, mina.linear);
}, function() {
  dfsButtonRect.animate({
  ...
  if (!messageIsShown) {
    messageIsShown = true;
    humane.log("Chose node for start", function() {
      return messageIsShown = false;
    });
  }
  return dfs_waitChoosingNode = true;
});

findCompsButtonStroke = paper.rect(0, 0, 190, 40, 5).attr({
  fill: "none",
  stroke: light,
  strokeWidth: 2
});

findCompsButtonRect = paper.rect(0, 0, 190, 40, 5).attr({
  fill: light,
  opacity: 0,
  strokeWidth: 2
});

findCompsButtonText = paper.text(24, 25, "Find Components").attr({
  fill: light,
  fontSize: 18,
  fontWeight: 200
});

paper.group(findCompsButtonRect, findCompsButtonStroke, findCompsButtonText).attr({
  transform: "t" + (screen.width - 200) + ",60"
}).hover(function() {
  findCompsButtonRect.animate({
    opacity: 1
  }, 200, mina.linear);
  return findCompsButtonText.animate({
    fill: dark
  }, 200, mina.linear);
}, function() {
  findCompsButtonRect.animate({
  ...
  if (!messageIsShown) {
    messageIsShown = true;
    humane.log("Finding connected components", function() {
      return messageIsShown = false;
    });
    return findComps();
  }
...
bfsButtonStroke = paper.rect(0, 0 + _y, 190, 40, 5).attr({
  fill: "none",
  stroke: light,
  strokeWidth: 2
});

bfsButtonRect = paper.rect(0, 0 + _y, 190, 40, 5).attr({
  fill: light,
  opacity: 0,
  strokeWidth: 2
});

bfsButtonText = paper.text(15, 25 + _y, "Breadth First Search").attr({
  fill: light,
  fontSize: 18,
  fontWeight: 200
});

paper.group(bfsButtonRect, bfsButtonStroke, bfsButtonText).attr({
  transform: "t" + (screen.width - 200) + ",60"
}).hover(function() {
  bfsButtonRect.animate({
    opacity: 1
  }, 200, mina.linear);
  return bfsButtonText.animate({
    fill: dark
  }, 200, mina.linear);
}, function() {
  bfsButtonRect.animate({
  ...
  return bfs_waitChoosingNode = true;
});

bfs = function(unit, adj) {
  var count, head, i, j, k, l, len, queue, ref, ref1, ref2, result, results, timeBfs, timer, visited;
  timeBfs = 1;
  result = [];
  queue = [];
  visited = [];
  for (i = j = 0, ref = adj.length; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
    queue[i] = 0;
  }
  head = 0;
  count = 0;
  ...
  while (head < count) {
    unit = queue[head++];
    result.push(unit + 1);
    for (i = k = 0, ref1 = adj.length; 0 <= ref1 ? k < ref1 : k > ref1; i = 0 <= ref1 ? ++k : --k) {
      if (adj[unit][i] && !visited[i]) {
        queue[count++] = i;
        visited[i] = true;
      }
    }
  ...
  for (l = 0, len = ref2.length; l < len; l++) {
    ...
    results.push(timers.push(timer));
  }
  return results;
};

startBfs = function(start) {
  resetGraphAnimation();
  if (nodes[start]) {
    nodes[start].animate({
      fill: '#93f8a5'
  ...
  return bfs(start, adjacencyMatrix);
};

findComps = function() {
  var comp, i, j, ref, results, timeOffset, visited;
  timeOffset = 0;
  visited = [];
  resetGraphAnimation();
  results = [];
  for (i = j = 0, ref = nodes.length; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
    if (visited.indexOf(i === -1)) {
      comp = dfs(i, false);
      visited.concat(comp);
      timeOffset++;
      results.push(comp.forEach(function(e) {
        return setTimeout((function() {
          return this.animate({
            fill: "red"
...
paper.click(function(e) {
  ...
  if (e.target.tagName === "svg") {
    node = paper.circle(e.offsetX, e.offsetY, 20).data("i", nodes.length).attr(nodeStyle).attr({
      transform: "s0,0"
    }).animate({
      transform: "s1,1"
    }, 1000, mina.elastic);
    t = paper.text(e.offsetX - 4, e.offsetY + 5, nodes.length + 1);
    node.data("t", t).drag(function(dx, dy, x, y) {
      t = this.data("t");
      t.attr({
        x: x - 4,
        y: y + 5
      ...
      var from, mouseupCoords, to;
      if (dfs_waitChoosingNode) {
        startDfs(this.data("i"));
        dfs_waitChoosingNode = false;
        return;
      } else if (bfs_waitChoosingNode) {
        startBfs(this.data("i"));
        bfs_waitChoosingNode = false;
        return;
      }
      mouseupCoords = {
      ...
      if (_.isEqual(mousedownCoords, mouseupCoords) || (this === prevNode)) {
        if (prevNode) {
          from = {
          ...
          to = {
            x: this.node.cx.baseVal.value,
            y: this.node.cy.baseVal.value
          };
          pathArray.push({
            from: prevNode,
            to: this
          ...
          return prevNode = void 0;
        } else {
          this.animate(prevNodeStyle, 1000, mina.elastic);
...
},{"humane-js":2,"react":157,"snapsvg":158,"underscore":160}],2:[function(require,module,exports){
/**
 * humane.js
...
var Danger = {
  ...
  dangerouslyReplaceNodeWithMarkup: function(oldChild, markup) {
    ...
    ("production" !== "dev" ? invariant(markup, 'dangerouslyReplaceNodeWithMarkup(...): Missing markup.') : invariant(markup));
    ("production" !== "dev" ? invariant(
...
var LocalEventTrapMixin = {
  trapBubbledEvent:function(topLevelType, handlerBaseName) {
    ("production" !== "dev" ? invariant(this.isMounted(), 'Must be mounted to trap events') : invariant(this.isMounted()));
...
function validateDangerousTag(tag) {
  if (!hasOwnProperty.call(validatedTagCache, tag)) {
    ("production" !== "dev" ? invariant(VALID_TAG_REGEX.test(tag), 'Invalid tag: %s', tag) : invariant(VALID_TAG_REGEX.test(tag)));
  ...
  for (var propName in propTypes) {
    if (propTypes.hasOwnProperty(propName)) {
      ...
      if (error instanceof Error && !(error.message in loggedTypeFailures)) {
        ...
        var addendum = getDeclarationErrorAddendum(this);
        ("production" !== "dev" ? warning(false, 'Failed propType: %s%s', error.message, addendum) : null);
      }
    }
  }
}
...
function createNodesFromMarkup(markup, handleScript) {
  var node = dummyNode;
  ("production" !== "dev" ? invariant(!!dummyNode, 'createNodesFromMarkup dummy not initialized') : invariant(!!dummyNode));
...
function getMarkupWrap(nodeName) {
  ("production" !== "dev" ? invariant(!!dummyNode, 'Markup wrapping node not initialized') : invariant(!!dummyNode));
...
},{"eve":159}],159:[function(require,module,exports){
// Copyright (c) 2013 Adobe Systems Incorporated. All rights reserved.
// 
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
